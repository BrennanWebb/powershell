...
# Function to search for members of a group, with FSP resolution
function GroupSearch {
    ...
    # Allow specifying default domain prefix in group name (e.g., DOMAIN\GroupName)
    ...
    foreach ($domain in $domains) {
        try {
            ...
            if ($groupMembers.Count -gt 0) {
                ...
                # Prompt if group has many members
    ...
    # Resolve group member identities
    $results = @()
    $counter = 0
    ...
    # Identify and resolve foreign security principals (FSPs)
    $counter = 0
